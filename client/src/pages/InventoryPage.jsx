...
import { Package, Search, Plus, AlertTriangle, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function InventoryPage() {
  ...
  const fetchProducts = async (page = 1) => {
    ...
    try {
      const res = await api.get('/inventory/products', {
        params: { page, limit: 10, search: search || undefined, lowStock: showLowStock || undefined }
      });
  ...
  useEffect(() => { fetchProducts(); }, [showLowStock]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <form onSubmit={(e) => { e.preventDefault(); fetchProducts(1); }} className="relative flex-1">
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${showLowStock ? 'bg-red-100 text-red-700 border border-red-200' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
          >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-right px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Cost</th>
                ...
                {products.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors cursor-pointer">
                      ...
                      <td className="px-5 py-4">
                        <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">{p.category_name || '-'}</span>
                      </td>
                      <td className="px-5 py-4 text-right font-medium">{formatCurrency(p.unit_price)}</td>
                      <td className="px-5 py-4 text-right text-gray-500">{formatCurrency(p.cost_price)}</td>
                      <td className="px-5 py-4 text-right font-medium">{p.quantity_in_stock} {p.unit}</td>
                      <td className="px-5 py-4 text-center">
                        {isLow ? (
                          <span className="px-2 py-1 bg-red-100 text-red-700 rounded-full text-xs font-medium flex items-center gap-1 justify-center">
        ...
        {pagination.totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-gray-200">
            ...
            <div className="flex gap-2">
              <button onClick={() => fetchProducts(pagination.page - 1)} disabled={pagination.page <= 1} className="p-2 rounded-lg border border-gray-200 hover:bg-gray-50 disabled:opacity-50"><ChevronLeft className="w-4 h-4" /></button>
              <button onClick={() => fetchProducts(pagination.page + 1)} disabled={pagination.page >= pagination.totalPages} className="p-2 rounded-lg border border-gray-200 hover:bg-gray-50 disabled:opacity-50"><ChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
        )}
      </div>
